/**
 * Commit Shards v2 — Pure SVG generator (browser-safe, no Node.js deps)
 *
 * Extracted from generator/shard.ts for use in the web app.
 */
...
function getColorFamily(hue: number): string {
  ...
  if (hue < 75) return 'Citrine';
  if (hue < 105) return 'Peridot';
  if (hue < 135) return 'Emerald';
  if (hue < 165) return 'Jade';
  if (hue < 195) return 'Sapphire';
  if (hue < 225) return 'Cobalt';
  if (hue < 255) return 'Tanzanite';
  if (hue < 285) return 'Amethyst';
  if (hue < 315) return 'Garnet';
  return 'Rose';
}

/**
 * Advance the RNG through one shardVertices() call sequence.
 * Must exactly mirror the RNG calls in shardVertices() above.
 */
function advanceVertexRng(rng: ShardRNG): void {
  rng.next(); rng.next(); rng.next(); // baseOff, w1, w2
  if (rng.next() > 0.3) { rng.next(); rng.next(); rng.next(); } // extra vertex 1
  if (rng.next() > 0.5) { rng.next(); rng.next(); rng.next(); } // extra vertex 2
}

/**
 * Extract deterministic visual traits from a commit SHA.
 * Mirrors the exact RNG call sequence of generateShardSVG so traits
 * are guaranteed to match the rendered crystal.
 */
export function extractShardTraits(
  config: ShardConfig,
): Array<{ trait_type: string; value: string | number }> {
  const { commitSha, repo, size = 800 } = config;
  ...
  const shardCount = 6 + (bytes[3] % 7);

  // ── Accent palette (2 RNG calls, same as generator) ──
  const accentChoice = rng.next();
  rng.next(); // accent offset value (consumed by rng.range in generator)
  const palette = accentChoice > 0.5 ? 'Analogous' : 'Complementary';

  // ── Advance through primary shards to reach secCount ──
  for (let i = 0; i < shardCount; i++) {
    rng.next(); // angle jitter
    rng.next(); // length
    rng.next(); // width
    advanceVertexRng(rng);
    rng.next(); // useAccent
    rng.next(); // hue adjustment
    rng.next(); // sat
    rng.next(); // lit
    rng.next(); // opacity
  }
  ...
  // ── Advance through secondary shards to reach starfield ──
  for (let i = 0; i < secCount; i++) {
    rng.next(); // angle
    rng.next(); // dist
    rng.next(); // length
    rng.next(); // width
    rng.next(); // angle adjustment
    advanceVertexRng(rng);
    rng.next(); // hue
    rng.next(); // sat
    rng.next(); // lit
  ...
  // ── Star count (buildStarfield first call) ──
  const starCount = 80 + rng.int(0, 60);

  // ── Classify traits ──
  const intensity = baseSat < 66 ? 'Muted' : baseSat < 77 ? 'Vibrant' : 'Intense';
  const starDensity = starCount < 100 ? 'Sparse' : starCount < 120 ? 'Normal' : 'Dense';

  return [
    { trait_type: 'Crystals', value: shardCount },
    { trait_type: 'Inner Shards', value: secCount },
    { trait_type: 'Color Family', value: getColorFamily(baseHue) },
    { trait_type: 'Intensity', value: intensity },
    { trait_type: 'Palette', value: palette },
    { trait_type: 'Starfield', value: starDensity },
  ];
}
